...
async function getGoogleUser({
  ...
  const {
    tokens
  } = await oauth2Client.getToken(code); // Fetch the user's profile with the access token and bearer

  const googleUser = await external_axios_default().get(`https://www.googleapis.com/oauth2/v1/userinfo?alt=json&access_token=${tokens.access_token}`, {
    headers: {
...
var __webpack_exports__ = __webpack_require__.X(0, [381,286], () => (__webpack_exec__(7098)));
