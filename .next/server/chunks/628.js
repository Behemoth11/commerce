...
const handle_put = async (req, res) => {
  ...
  if (req.user.role != "admin") data.representation = "none"; // console.log("the old images are :", old_images)
...
const handle_get = async (req, res) => {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    switch (key) {
      ...
      case "categories":
        const categories = (0,UtilityFunctions/* string_and_array_to_array */.j6)(query.categories);
...
const handle_post = async (req, res) => {
  const data = req.body;
  const error = [];
  const [pr_image_url, all_pr_image_url] = await Promise.all([await uploadMany(data["presentationImage"], error, {}).catch(err => error.push(err.message)), await uploadMany(data["images"], error, {}).catch(err => error.push(err.message))]);
  if (req.user.role != "admin") data.representation = "none";
  /************mongoDb upload***************/

  const mongo_response = await models/* Product.insertMany */.xs.insertMany([{
    pr_image_url,
    //array
    tags: data.tags,
    //array
